...
const worker = new Worker<WebhookJobData>(
    ...
    async job => {
        ...
        const webhook = await db.webhook.findUnique({ where: { id: webhookId }, select: { id: true, url: true, secretHash: true, isActive: true, events: true } })
        ...
        try {
            const res = await fetch(webhook.url, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-VeriSure-Event': event, 'X-VeriSure-Signature': `sha256=${sig}`, 'X-VeriSure-Delivery': job.id ?? 'unknown', 'User-Agent': 'VeriSure-Webhook/1.0' },
                body,
        ...
        await db.webhookDelivery.create({ data: { webhookId, event, payload: payload as object, statusCode, responseBody: responseBody?.slice(0, 500) ?? null, duration, attempt: (job.attemptsMade ?? 0) + 1, success } })
        await db.webhook.update({ where: { id: webhookId }, data: { lastDeliveredAt: new Date(), failureCount: success ? 0 : { increment: 1 } } })
...
process.on('SIGTERM', async () => { logger.info('webhook-worker: stopping'); await worker.close(); process.exit(0) })